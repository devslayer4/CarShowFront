...
import { Car } from "@/components/cars/AllCars";
...
export function AppointmentForm({ car, onClose }: AppointmentFormProps) {
  ...
  return (
    <dialog open className="fixed inset-0 flex items-center justify-center z-50">
      <button className="absolute inset-0 bg-gray-900 opacity-50" onClick={onClose} aria-label="Close overlay" />
        ...
        <form className="space-y-4" onSubmit={handleSubmit}>
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <Input id="carId" name="carId" required defaultValue={car?.id} readOnly />
            </div>
            <div>
